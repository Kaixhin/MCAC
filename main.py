...
parser.add_argument('--num-seeds', type=int, default=50, metavar='SEEDS', help='Number of seed genomes to evolve that satisfy the MC')
...
parser.add_argument('--epochs', type=int, default=30, metavar='EPOCHS', help='Adversarial training epochs')
args = parser.parse_args()
assert args.num_seeds > args.evaluation_size // 2
...
if __name__ == '__main__':
  fixed_latent = torch.randn(args.batch_size, args.latent_size, device=device)
  ...
  viable_pop = deque(evolve_seed_genomes(rand_pop, args.num_seeds, args.latent_size, args.learning_rate, args.batch_size, args.epochs, fixed_latent, args.mutation_rate, device))
  ...
  for _ in range(args.max_epochs):
    ...
    for i, child in enumerate(children):  # TODO: Somehow make sure there are both generators and discriminators?
      ...
      num_evaluations += 1
      mc_satisfied = evaluate_mc(generator, discriminator, args.criterion_threshold, num_evaluations, fixed_latent)
      if mc_satisfied:
